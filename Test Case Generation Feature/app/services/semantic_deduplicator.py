...
class DeduplicationConfig:
    ...
    Every parameter is documented and overridable — no magic numbers are
    embedded in the algorithm itself.

    Follows the same pattern as ModelConfig in model_config.py.
    """
    ...
    # Has no effect on output — only on throughput/memory.
    encode_batch_size: int = 64


# Singleton default — importable and overridable by callers.
...
class SemanticDeduplicator:
    ...
    1. Structural prefix normalization (data-driven, not hardcoded):
       ...
       signal to the embedding space — it only inflates pairwise similarities
    ...
    def _normalize_batch(
        ...
        Design rationale
        ----------------
        The common prefix is detected entirely from the data — it is NOT a
        ...
        for req in requirements:
            if req.lower().startswith(prefix_lower):
                ...
                # Never strip to an empty string — keep original if tail is trivial.
    ...
    def _derive_threshold(
        ...
        -------
        Sort similarities descending → they form a monotone decreasing curve.
        ...
        distance is the "elbow" — the boundary between the steep duplicate
        ...
        # Normalize x to [0, 1] — makes curvature detection scale-invariant.
        ...
        if line_len < 1e-9:
            # Degenerate: all similarities are equal — no meaningful boundary.
            return self.config.fallback_threshold, "fallback"
        ...
        # essentially linear and the elbow is unreliable — fall back.
