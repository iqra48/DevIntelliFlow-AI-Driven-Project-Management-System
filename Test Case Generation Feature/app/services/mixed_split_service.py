...
from app.shared.llm.output_guard import OutputValidationError, guarded_llm_call
...
# emits only the FR — creating a phantom duplicate of the original MIXED
...
def _output_token_budget(count: int) -> int:
    """
    Each split produces two IEEE sentences ≈ 40 tokens each = ~80 tokens per item.
...
async def _split_chunk(requirements: List[str]) -> Dict[str, Dict[str, str]]:
    ...
    Returns a dict mapping 1-based string index → {"fr": ..., "nfr": ...}.
        ...
        try:
            ...
            for idx, orig in enumerate(chunk, 1):
                ...
                if not entry or not isinstance(entry, dict):
                    logger.warning(
                        "mixed_split no entry for req index=%d req=%r — using original as FR",
                        idx,
                        orig[:60],
                    )
                ...
                if not fr_valid:
                    logger.warning(
                        "mixed_split FR failed IEEE check fr=%r — using original as FR",
                        fr[:80],
                    )
                    fr = orig
                ...
                if not nfr_valid:
                    logger.warning(
                        "mixed_split NFR failed IEEE check nfr=%r — omitting NFR",
                        nfr[:80],
                    )
                    nfr = ""
            ...
            logger.error(
                "mixed_split chunk failed (%s) — using originals as FR for %d requirements",
                exc,
                len(chunk),
            )
            ClassificationEventLogger.log_mixed_split(
